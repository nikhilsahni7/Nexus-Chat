...
export function ConversationList({ conversations }: ConversationListProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <ScrollArea className="flex-grow p-2 sm:p-4 space-y-2 sm:space-y-4 overflow-y-auto">
          ...
          <Link href={`/chat/${conversation.id}`} key={conversation.id}>
            <div className="flex items-center p-2 sm:p-3 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors">
              <Avatar className="h-10 w-10 sm:h-12 sm:w-12 mr-2 sm:mr-3">
                ...
                />
                <AvatarFallback>{conversation.name?.[0] || "N"}</AvatarFallback>
              </Avatar>
      ...
      {/* Button Section */}
      <div className="p-2 sm:p-4 border-t border-gray-200 sticky bottom-0 bg-white z-10">
        <Button
